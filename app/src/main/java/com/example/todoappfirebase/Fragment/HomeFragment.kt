...
import com.example.todoappfirebase.ultils.todoadapter.ToDoAdapter
...
class HomeFragment : Fragment(), AddToDoFragment.DialogNextBtnClickListener,
    ...
    private lateinit var adap: ToDoAdapter
    private lateinit var ds:MutableList<ToDoData>
    ...
    private fun logOut() {
        binding.btnLogOut.setOnClickListener {
            val options = arrayOf("LogOut","Exit")

            val builder = AlertDialog.Builder(requireContext())
            builder.setTitle("Are you want?")
    ...
    private fun init(view: View) {
        ...
        ds = mutableListOf()
        adap = ToDoAdapter(ds)
        adap.setListener(this)
        binding.rvList.adapter = adap
    }

    override fun onSaveTask(todo: String, todoEt: TextInputEditText) {
        databaseRef.push().setValue(todo).addOnCompleteListener {
            if(it.isSuccessful){
                Toast.makeText(context, "Todo saved Successfully", Toast.LENGTH_LONG).show()
            }else{
    ...
    override fun onEditClicked(toDoData: ToDoData) {
        if(popUpFragment != null){
            childFragmentManager.beginTransaction().remove(popUpFragment!!).commit()
        }
